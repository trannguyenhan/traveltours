<template>
  <v-container class="px-4 mt-6 pb-0">
    <H3TRating class="mt-n8 pb-0" :rating="calcRating()" :reviews-quantity="calcRating(false)" />

    <H3THighlights class="py-1" :highlights="tour.dest.description" v-on="$listeners" />
    <H3TDestinations class="py-1" :destinations="tour.dest.address_detail" />

    <H3TTravelStyle class="py-1" :travel-style="tour.categories" v-on="$listeners" />
    <H3TDuration class="py-2" :duration="tour.range" />
    <H3TPrice :old-price="tour.price.adult * (1 + (Math.random())).toFixed(2)" :price="tour.price.adult" />
  </v-container>
</template>
